    ...
    this.set = function(property,value,cb) {
        ...
        chrome.storage.sync.set(
            ...
            function sentToStorage() {
                console.log("SET app.session."+property+" = "+value)
                if(typeof app.session.after[property] === 'function') app.session.after[property](value)
                if(typeof cb === 'function') cb(value)
            }
        )
        return this[property]
    }
    this.get = function(property,cb) {
        var that = this
        chrome.storage.sync.get(property, function receivedPropertyFromStorage(storage) {
            console.log("GET app.session."+property+" = "+storage[property])
            that[property] = storage[property]
            if(typeof app.session.after[property] === 'function') app.session.after[property](that[property])
