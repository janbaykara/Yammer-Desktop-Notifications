...
function handleNotificationEvents(notificationId,message) {
    ...
    chrome.notifications.onClicked.addListener(function onNotificationClicked(id) {
        if (id === notificationId) {
            ...
            // Search windows & tabs for yammer pages (prioritise current tab)
            (function searchCurrentWindow() {
                chrome.windows.getCurrent({}, function(w) {
                        getWindowTabs(w, function(moreTabs) {
                            yammerTabs = yammerTabs.concat(moreTabs)
                            nowSearchTheRest()
                        })
                })
            })();
            function nowSearchTheRest() {
                chrome.windows.getAll({}, function(wArr) {
                    _.forEach(wArr, function(w,i) {
                        getWindowTabs(w, function(moreTabs) {
                            yammerTabs = yammerTabs.concat(moreTabs)
                            if(i+1 === wArr.length) takeMeToYourLeader(yammerTabs,message)
                        })
                    })
                })
            }
...
function getWindowTabs(window,cb) {
    chrome.tabs.query({
        windowId: window.id,
        url: "*://*.yammer.com/*"
