...
var app = {
    ...
    init: function() {
        ...
        // Spark off the listeners
        _.each(app.detectors, function(listener) {
            listener()
        })
    },
            ...
            chrome.webRequest.onCompleted.addListener(function receiveHTTPRequest(req) {
                if (
                    ...
                    $.get(req.url+"&"+app.config.ignoreToken, function analyseHTTPRequest(res) {
                        res.detector = "lookForHTTPRequests"
                        app.engine.initiateNotificationPipeline(res)
                    })
    ...
    engine: {
        ...
        initiateNotificationPipeline: function(res) {
            if(typeof app.session.yammer_uid === 'undefined') {
                app.session.set("yammer_uid",res.data.meta.current_user_id)
            }
            app.engine.prepareMessageData(res)
        },

        prepareMessageData: function(res) {
        ...
        buildNotification: function(res) {
            ...
            if(typeof app.session.lastMsgId !== 'undefined')
                app.engine.sendNotification(message)
        },
        ...
        sendNotification: function(message) {
            ...
            chrome.notifications.create(
                message.notification_id,
                {
                    ...
                    contextMessage: "Click to reply · "+message.date_object.toLocaleTimeString(),
                    title: message.person.full_name + " yammer'd you",
                    message: message.body.plain,
                    priority: 1
                }, function callback(notificationId) {
                    app.engine.handleNotificationEvents(notificationId, message)
                }
            )
        },

        handleNotificationEvents: function(notificationId,message) {
            // Go to message
            chrome.notifications.onClicked.addListener(function(id) {
                if (id === notificationId) app.engine.onNotificationClicked(message)
            })
        },

        onNotificationClicked: function(message) {
            ...
            chrome.windows.getCurrent({}, function(w) {
                chrome.tabs.query({
                    ...
                    app.engine.takeMeToYourLeader((tabs || []),message)
                })
            })
        },

        takeMeToYourLeader: function(yammerTabs,message) {
